    ...
    <section class="generator-card">
        ...
        <div class="source-toolbar">
            ...
            <div class="source-actions">
                <el-button :disabled="!sources.length" @click="useRecommended">
                    使用推荐配置
                </el-button>
                <el-button :disabled="!sources.length" @click="selectAll">
                    全选
                </el-button>
                <el-button :disabled="!selectedIds.length" @click="clearSelection">
                    清空
                </el-button>
            </div>
        </div>
        ...
        <div class="config-result">
            <div class="result-heading">
                <div>
                    <h3>生成的配置</h3>
                    <span v-if="selectedSources.length">
                        已选择 {{ selectedSources.length }} 个站点
                    </span>
    ...
    watch: {
        sources: {
            handler(value) {
                ...
                if (!this.selectedIds.length && value.length) {
                    this.useRecommended();
                }
            },
            immediate: true,
...
.source-grid :deep(.el-checkbox.is-bordered.is-checked) {
    background: #eef4ff;
    border-color: #165dff;
}

.source-grid :deep(.el-checkbox__label) {
    display: flex;
    min-width: 0;
    padding-left: 10px;
    flex-direction: column;
...
.result-heading :deep(.el-button .el-icon) {
    margin-right: 6px;
}
