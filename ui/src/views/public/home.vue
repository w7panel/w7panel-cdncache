    ...
    <div class="public-home">
        ...
        <main id="top">
            ...
            <div class="page-container">
                ...
                <section
                    v-if="pageSetting.markdown.trim()"
                    class="content-card custom-content"
                >
...
export default {
    name: "PublicHome",
    components: {
        MirrorConfigGenerator,
    },
    data() {
        return {
            loading: true,
            sources: [],
            pageSetting: createPageSetting(),
        };
    },
    computed: {
        customContentHtml() {
        ...
        icpBeianLink() {
            return this.pageSetting.icp_link
                ? safeHttpLink(this.pageSetting.icp_link)
                : ICP_BEIAN_URL;
        },
        policeBeianLink() {
    ...
    methods: {
        async load() {
            try {
                const response = await getPublicSiteList(true);
        ...
        normalizeSources(data) {
            ...
            Object.entries(data || {}).forEach(([group, setting]) => {
                ...
                group
                    ...
                    .filter(Boolean)
                    .forEach((domain) => {
                        const protocol = domain === window.location.host
                        ...
                        sourceMap.set(domain, {
                            ...
                            domain,
                            url: `${protocol}//${domain}`,
                            origin: setting?.storage_source?.endpoint || "",
                        });
                    });
            });
...
.public-header {
    ...
    left: 0;
    color: #fff;
    background: linear-gradient(180deg, rgb(0 0 0 / 76%) 0%, rgb(0 0 0 / 0%) 100%);
}
...
.hero {
    ...
    color: #fff;
    background:
        radial-gradient(circle at 72% 28%, rgb(75 125 255 / 32%), transparent 32%),
        linear-gradient(125deg, #0b1530 0%, #101e44 54%, #102c5a 100%);
}
...
.hero::after {
    ...
    width: 680px;
    height: 680px;
    background: repeating-linear-gradient(135deg, rgb(255 255 255 / 5%) 0 1px, transparent 1px 19px);
    border-radius: 50%;
    content: "";
}
...
.hero-glow-one {
    ...
    width: 240px;
    height: 240px;
    background: rgb(49 112 255 / 18%);
    border: 1px solid rgb(107 151 255 / 22%);
}
...
.hero-badge {
    ...
    padding: 7px 12px;
    color: #8eb6ff;
    background: rgb(50 111 255 / 13%);
    border: 1px solid rgb(91 140 255 / 24%);
    border-radius: 999px;
    font-size: 11px;
    font-weight: 700;
...
.primary-action {
    color: #fff !important;
    background: #3478f6;
    box-shadow: 0 12px 26px rgb(27 91 230 / 34%);
}
...
.content-card {
    ...
    border-radius: 20px;
    box-shadow: 0 24px 60px rgb(33 56 97 / 8%);
}
